            ...
            with self.subTest(image=file_name):
                for i, actual_sale in enumerate(actual_sales):
                    with self.subTest(sale_index=i):
                        self.assertEqual(actual_sale.name, expected_sales[i].name, f"{expected_sales[i].name} actual vs expected")
                        self.assertEqual(actual_sale.price, expected_sales[i].price, f"{expected_sales[i].name} actual vs expected")
                        self.assertEqual(actual_sale.auction_number, expected_sales[i].auction_number, f"{expected_sales[i].name} actual vs expected")
                        self.assertEqual(actual_sale.price_type, expected_sales[i].price_type, f"{expected_sales[i].name} actual vs expected")
    ...
    def get_sales(self) -> dict[list[DofusPriceModel]]:
        ...
        return {
            'cheeken.png': [
                ...
                DofusPriceModel(name='Cheeken Cloaca', price=19993.0, auction_number=3, price_type='10'),
                DofusPriceModel(name='Cheeken Cloaca', price=180000.0, auction_number=4, price_type='100'),
                DofusPriceModel(name='Cheeken Cloaca', price=2449996.0, auction_number=5, price_type='1000'),
            ],
            'belteen.png': [
                ...
                DofusPriceModel(name='Belteen', price=6000000.0, auction_number=4, price_type='1'),
            ...
            'ice_knight_map.png': [
                DofusPriceModel(name="Ice Knight's Map", price=2369776.0, auction_number=1, price_type='average'),
                DofusPriceModel(name="Ice Knight's Map", price=1799980.0, auction_number=2, price_type='1'),
            ],
            'rhinteele_ring.png': [
